...
const KanbanBoard = () => {
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/calendar/action-items/${editForm.id}`,
        {
          task: editForm.task,
  ...
  const handleAddSave = async () => {
    ...
    try {
      await axios.post(
        `${API_BASE_URL}/calendar/action-items`,
        {
          task: addForm.task,
                  ...
                  {(provided, snapshot) => (
                    <Box
                      ...
                      sx={{
                        ...
                        bgcolor: snapshot.isDraggingOver 
                          ? alpha(config.gradient.includes('#667eea') ? '#667eea' : 
                                  config.gradient.includes('#f093fb') ? '#f093fb' : '#4facfe', 0.08)
                          : 'transparent',
                    ...
                    >
                      ...
                      {tasks.map((task, index) => {
                        ...
                        const isMyTask = !showOnlyMyTasks && filterUserName && task.owner && 
                                    ...
                                    {task.due_date && (() => {
                                      ...
                                      return (
                                        <Chip
                                          ...
                                          sx={{
                                            height: 26,
                                            bgcolor: isOverdue ? '#ffebee' : isToday ? '#e3f2fd' : 'grey.100',
                                            color: isOverdue ? '#d32f2f' : isToday ? '#1976d2' : 'text.secondary',
                                            '& .MuiChip-icon': {
                                              color: isOverdue ? '#d32f2f' : isToday ? '#1976d2' : 'text.secondary',
                                            },
                                            '& .MuiChip-label': { fontSize: '0.75rem' },
      ...
      >
        <DialogTitle sx={{ pb: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6" fontWeight={600}>Edit Action Item</Typography>
          </Box>
        </DialogTitle>
