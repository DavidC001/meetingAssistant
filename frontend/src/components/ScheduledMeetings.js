...
const ScheduledMeetings = () => {
  ...
  const linkMeetingToScheduled = async (meetingId) => {
    try {
      await api.post(
        `/api/v1/scheduled-meetings/${selectedScheduledMeeting.id}/link/${meetingId}`
      );
            ...
            {showUploadPrompt && !meeting.linked_meeting_id && (
              <Grid item xs={12}>
                <Alert severity="warning" sx={{ mt: 1 }}>
                  <Typography variant="body2">
                    This meeting has occurred. Please upload the recording to keep your records complete.
                  </Typography>
                  <Button
                    size="small"
                    variant="contained"
    ...
    <Box>
      ...
      <Tabs value={tabValue} onChange={(e, newValue) => setTabValue(newValue)} sx={{ mb: 3 }}>
        ...
        <Tab 
          ...
          icon={needsUploadMeetings.length > 0 ? <Chip label={needsUploadMeetings.length} size="small" color="warning" /> : null}
          iconPosition="end"
        />
      </Tabs>
            ...
            <Box>
              {needsUploadMeetings.length === 0 ? (
                <Alert severity="success">
                  All recent meetings have recordings linked! 🎉
                </Alert>
              ) : (
                <>
...
const LinkMeetingDialog = ({ open, onClose, scheduledMeeting, onLink }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle>
        Link Recording to: {scheduledMeeting?.title}
      </DialogTitle>
      <DialogContent>
        <TextField
          fullWidth
                ...
                <React.Fragment key={meeting.id}>
                  <ListItem>
                    <ListItemText
                      ...
                      secondary={`Status: ${meeting.status} | Created: ${new Date(meeting.created_at).toLocaleString()}`}
                    />
                    <ListItemSecondaryAction>
                      <Button
                        variant="contained"
