  ...
  const filteredData = React.useMemo(() => {
    return {
      nodes: graphData.nodes.filter(
        (node) => visibleTypes.includes(node.type) && !hiddenNodes.has(node.id)
      ),
      links: graphData.links.filter((link) => {
        ...
        return (
          graphData.nodes.some(
            (n) => n.id === sourceId && visibleTypes.includes(n.type) && !hiddenNodes.has(n.id)
          ) &&
          graphData.nodes.some(
            (n) => n.id === targetId && visibleTypes.includes(n.type) && !hiddenNodes.has(n.id)
          )
        );
      }),
    };
                ...
                <Box>
                  {selectedNode.data.meeting_date && (
                    <Typography variant="body2" sx={{ mb: 1 }}>
                      <strong>Date:</strong>{' '}
                      {new Date(selectedNode.data.meeting_date).toLocaleDateString()}
                    </Typography>
                  )}
              ...
              {selectedNode.type === 'meeting' && selectedNode.data && (
                <Box sx={{ mt: 2 }}>
                  <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.85rem' }}>
                    Tip: Add meeting references in notes using #meeting-{selectedNode.data.id} or [[
                    {selectedNode.data.id}]]
                  </Typography>
                </Box>
              )}
