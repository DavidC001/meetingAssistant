...
const MeetingsBrowser = () => {
  ...
  const filteredMeetings = useMemo(() => {
    ...
    if (filters.dateRange) {
      ...
      if (filterDate) {
        result = result.filter((m) => new Date(m.meeting_date || m.created_at) >= filterDate);
      }
    }
  ...
  const handleBulkMove = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Moved ${selectedMeetings.length} meeting(s) to ${bulkFolder}`,
        severity: 'success',
      });
  ...
  const handleBulkAddTags = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Added tags to ${selectedMeetings.length} meeting(s)`,
        severity: 'success',
      });
      ...
      setSnackbar({
        open: true,
        message: 'Failed to add tags to some meetings',
        severity: 'error',
      });
    } finally {
      setProcessing(false);
