  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const todayCount = meetings.filter(
        (m) => new Date(m.meeting_date || m.created_at) >= todayStart
      ).length;

      const thisWeekCount = meetings.filter(
        (m) => new Date(m.meeting_date || m.created_at) >= weekStart
      ).length;
  ...
  return (
    <Fade in timeout={500}>
      <Box>
        ...
        {/* Upload FAB */}
        <UploadFAB processingCount={stats.processing} onUploadComplete={handleMeetingsUpdate} />
      </Box>
    </Fade>
  );
};
