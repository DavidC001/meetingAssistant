  ...
  const handleContentDrop = (e) => {
    ...
    if (itemData) {
      ...
      if (match) {
        ...
        const endOfWorkedOn = nextSectionMatch ? insertPosition + nextSectionMatch.index : content.length;
      ...
      <Paper elevation={2} sx={{ p: 3 }}>
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          ...
          <Box display="flex" alignItems="center" gap={1}>
            <IconButton onClick={() => navigate('/diary/statistics')} color="primary" title="View Statistics">
              <StatisticsIcon />
            </IconButton>
                  ...
                  <Collapse in={actionItemsExpanded}>
                    ...
                    {actionItemsSummary ? (
                      <Box mt={2}>
                        ...
                        {/* In Progress Items */}
                        {filterActionItems(actionItemsSummary.in_progress_items || []).length > 0 && (
                          <Box mb={2}>
                            <Box display="flex" alignItems="center" gap={1} mb={1}>
                              <InProgressIcon color="primary" fontSize="small" />
                              <Typography variant="subtitle2" color="primary">
                                In Progress ({filterActionItems(actionItemsSummary.in_progress_items || []).length})
                              </Typography>
                            </Box>
                            <List dense>
                              {filterActionItems(actionItemsSummary.in_progress_items || []).map((item) => (
                                <ListItem 
                                  key={item.id}
                                  draggable
                                ...
                                >
                                  ...
                                  <ListItemText
                                    primary={item.task}
                                    secondary={`${item.owner || 'Unassigned'} ${item.due_date ? `• Due: ${item.due_date}` : ''}`}
                          ...
                          <Box mb={2}>
                            <Box display="flex" alignItems="center" gap={1} mb={1}>
                              ...
                              <Typography variant="subtitle2" color="warning.main">
                                Due Today ({filterActionItems(actionItemsSummary.created_items || []).length})
                              </Typography>
                            </Box>
                            <List dense>
                              {filterActionItems(actionItemsSummary.created_items || []).map((item) => (
                                <ListItem 
                                  key={item.id}
                                  draggable
                                  onDragStart={(e) => handleActionItemDragStart(e, item)}
                                ...
                                >
                                  ...
                                  <ListItemText
                                    primary={item.task}
                                    secondary={`${item.owner || 'Unassigned'} ${item.priority ? `• ${item.priority}` : ''}`}
                                    primaryTypographyProps={{ variant: 'body2' }}
                                  />
                                </ListItem>
                              ))}
                          ...
                          <Box mb={2}>
                            <Box display="flex" alignItems="center" gap={1} mb={1}>
                              ...
                              <Typography variant="subtitle2" color="success.main">
                                Completed Today ({filterActionItems(actionItemsSummary.completed_items || []).length})
                              </Typography>
                            </Box>
                            <List dense>
                              {filterActionItems(actionItemsSummary.completed_items || []).map((item) => (
                                <ListItem 
                                  key={item.id}
                                  draggable
                                  onDragStart={(e) => handleActionItemDragStart(e, item)}
                          ...
                          <Box mb={2}>
                            <Box display="flex" alignItems="center" gap={1} mb={1}>
                              ...
                              <Typography variant="subtitle2" color="info.main">
                                New Items ({filterActionItems(actionItemsSummary.created_items || []).length})
                              </Typography>
                            </Box>
                            <List dense>
                              {filterActionItems(actionItemsSummary.created_items || []).map((item) => (
