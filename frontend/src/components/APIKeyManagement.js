    ...
    <Box>
      ...
      <Card>
        <CardContent>
          {apiKeys.length === 0 ? (
            <Alert severity="info">
              No API keys configured. Click "Add API Key" to create your first API key configuration.
            </Alert>
          ) : (
            <List>
              {apiKeys.map((apiKey) => {
                      ...
                      <Box>
                        ...
                        {isEnvironmentKey && (
                          <Typography variant="caption" color="textSecondary" style={{ fontStyle: 'italic' }}>
                            This API key is loaded from environment variables and cannot be edited here.
                          </Typography>
                        )}
                      </Box>
                    }
