...
const api = {
  ...
  // Speaker API
  addSpeaker: (meetingId, speaker) =>
    client.post(`/api/v1/meetings/${meetingId}/speakers`, speaker),
  ...
  updateSpeaker: (speakerId, speaker) =>
    client.put(`/api/v1/meetings/speakers/${speakerId}`, speaker),
  deleteSpeaker: (speakerId) => client.delete(`/api/v1/meetings/speakers/${speakerId}`),

  // Action Item API
  addActionItem: (transcriptionId, actionItem) =>
    client.post(`/api/v1/meetings/transcriptions/${transcriptionId}/action-items`, actionItem),
  updateActionItem: (itemId, actionItem) =>
    client.put(`/api/v1/meetings/action-items/${itemId}`, actionItem),
  ...
  globalChat: {
    ...
    createSession: (title, tags, filterFolder, filterTags) =>
      client.post('/api/v1/global-chat/sessions', {
        title,
        tags,
    ...
    updateSession: (sessionId, title, tags, filterFolder, filterTags) =>
      client.put(`/api/v1/global-chat/sessions/${sessionId}`, {
        title,
        tags,
        filter_folder: filterFolder,
        filter_tags: filterTags,
      }),
    sendMessage: (sessionId, message, chatHistory, topK) =>
      client.post(`/api/v1/global-chat/sessions/${sessionId}/messages`, {
        message,
