...
def get_scheduled_meeting_by_calendar_event_id(db: Session, event_id: str):
    ...
    return db.query(models.ScheduledMeeting).filter(
        models.ScheduledMeeting.google_calendar_event_id == event_id
    ).first()
...
def get_user_mappings(db: Session, skip: int = 0, limit: int = 100):
    ...
    return db.query(models.UserMapping).filter(
        models.UserMapping.is_active == True
...
def get_user_mapping_by_name(db: Session, name: str):
    ...
    return db.query(models.UserMapping).filter(
        models.UserMapping.name.ilike(name),
        models.UserMapping.is_active == True
    ).first()
...
def get_user_mapping_by_email(db: Session, email: str):
    ...
    return db.query(models.UserMapping).filter(
        models.UserMapping.email.ilike(email),
        models.UserMapping.is_active == True
    ).first()
...
def create_user_mapping(db: Session, user_mapping: schemas.UserMappingCreate):
    ...
    existing = db.query(models.UserMapping).filter(
        models.UserMapping.name.ilike(user_mapping.name),
        models.UserMapping.is_active == True
    ).first()
