...
from ..integrations.export import export_meeting_data
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not resume_stage or resume_stage == "conversion":
            ...
            if metadata_checkpoint:
                estimated_duration_minutes = metadata_checkpoint.get("estimated_duration_minutes", 0.0) or 0.0
        ...
        if not resume_stage or resume_stage == "conversion":
            ...
            checkpoint_manager.save_checkpoint(
                "conversion",
                {"wav_audio_path": str(wav_audio_path), "completed": True},
        ...
        if not resume_stage or resume_stage in ["conversion", "diarization"]:
            ...
            if diarization_checkpoint and checkpoint_manager.validate_checkpoint("diarization"):
                ...
                meetings_crud.update_meeting_processing_details(
                    ...
                    processing_logs=[
                        f"Diarization loaded from checkpoint - {len(diarization_segments)} segments found"
                    ],
                )
            else:
                ...
                unique_speakers = {segment["speaker"] for segment in diarization_segments}
                ...
                meetings_crud.update_meeting_processing_details(
                    ...
                    overall_progress=50.0,
                    processing_logs=[
                        f"Diarization completed - {len(diarization_segments)} segments found, {len(unique_speakers)} speakers identified"
                    ],
                )
        ...
        if not resume_stage or resume_stage in ["conversion", "diarization", "transcription"]:
            ...
            if transcription_checkpoint and checkpoint_manager.validate_checkpoint("transcription"):
                ...
                checkpoint_manager.save_checkpoint(
                    "transcription",
                    {"full_transcript": full_transcript, "dominant_language": dominant_language},
                    metadata={
                        "stage": "transcription",
...
{full_transcript}"""
                ...
                logger.info(f"Transcript word count: {transcript_word_count}, Duration: {duration_str} minutes")
        ...
        transcription_data = schemas.TranscriptionCreate(summary="\n".join(enhanced_summary), full_text=full_transcript)
