...
class DiarizationProgressTracker:
    ...
    def __init__(self, db: Session, audio_path: str, meeting_id: int):
        ...
        if self.audio_duration and self.avg_processing_rate:
            ...
            logger.info(f"Estimated diarization time: {self.estimated_total_time:.1f}s for {self.audio_duration:.1f}s audio")
        else:
    ...
    def get_progress_callback(self, external_callback: Optional[Callable] = None):
        """
        ...
        def progress_callback(base_progress: int, message: str):
            ...
            logger.debug(f"Diarization progress: {actual_progress}% (base: {base_progress}%, elapsed: {elapsed_time:.1f}s)")
    ...
    def finish_tracking(self, num_speakers: Optional[int] = None):
        """
        ...
        if self.audio_duration:
            try:
                ...
                logger.info(f"Diarization completed in {total_time:.1f}s for {self.audio_duration:.1f}s audio (rate: {processing_rate:.2f}x)")
...
def estimate_diarization_time(db: Session, audio_path: str) -> Optional[float]:
    """
