...
from ..base.retry import retry_api_call
...
class LLMConfig:
    ...
    api_key_env: str | None = None  # Legacy: environment variable name
    api_key: str | None = None  # New: actual API key value
    max_tokens: int = 4000
    timeout: int = 300
...
class OpenAIProvider(LLMProvider):
    ...
    def _get_token_param(self) -> dict[str, int]:
        ...
        if model in old_models_exact or (
            model.startswith("gpt-3.5")
            or (model.startswith("gpt-4") and "gpt-4o" not in model and "gpt-4-turbo" not in model)
        ):
    ...
    async def chat_completion(
        ...
        try:
            ...
            if message.tool_calls:
                ...
                return {"tool_calls": tool_calls, "message": message.content if message.content else ""}
    ...
    async def analyze_transcript(self, transcript: str, system_prompt: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not response.choices[0].message:
                self.logger.error("Invalid response structure from OpenAI: no message in choice")
            ...
            self.logger.info(
                f"OpenAI response received. Content length: {len(content) if content else 0}, finish_reason: {finish_reason}"
            )
            ...
            raise ConnectionError(
                "Failed to connect to OpenAI API. Please check your network connection and API configuration."
            )
    ...
    async def chat_completion(
        ...
        try:
            ...
            if message.get("tool_calls"):
                ...
                return {"tool_calls": tool_calls, "message": message.get("content", "")}
    ...
    async def analyze_transcript(self, transcript: str, system_prompt: str) -> dict[str, Any]:
        ...
        try:
            payload = {
                ...
                "format": "json",
                "stream": False,
                "options": {"num_ctx": self.config.max_tokens},
            }
...
def model_config_to_llm_config(model_config, purpose: str, db: Session) -> LLMConfig:
    ...
    return LLMConfig(
        provider=provider, model=model, base_url=base_url, api_key=api_key, max_tokens=model_config.max_tokens
    )
