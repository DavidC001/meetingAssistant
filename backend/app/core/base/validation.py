...
def validate_file_extension(
    filename: str, allowed_extensions: set[str] | None = None, error_message: str | None = None
) -> str:
    """
    ...
    if extension not in allowed_extensions:
        msg = (
            error_message or f"File type '{extension}' not supported. Allowed: {', '.join(sorted(allowed_extensions))}"
        )
...
def validate_file_size(file_size: int, max_size_mb: int = 500, error_message: str | None = None) -> int:
    """
...
def validate_required_fields(data: dict, required_fields: list[str], context: str = "request") -> None:
    """
    ...
    if missing:
        raise ValidationError(f"Missing required fields in {context}: {', '.join(missing)}", field=missing[0])


def validate_string_length(value: str, field_name: str, min_length: int = 0, max_length: int = 10000) -> str:
    """
...
def validate_positive_integer(value: Any, field_name: str, min_value: int = 1, max_value: int | None = None) -> int:
    """
...
def parse_iso_date(date_string: str, field_name: str = "date") -> datetime:
    ...
    raise ValidationError(f"Invalid date format for {field_name}. Expected ISO format (YYYY-MM-DD)", field=field_name)


def validate_date_range(start_date: datetime | None, end_date: datetime | None, field_name: str = "date range") -> None:
    """
    ...
    if start_date and end_date and start_date > end_date:
        raise ValidationError(f"Invalid {field_name}: start date must be before end date", field=field_name)
