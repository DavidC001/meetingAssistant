...
def search(search_query: schemas.SearchQuery, db: Session = Depends(get_db)):
    ...
    base_query = db.query(meeting_models.Meeting).filter(meeting_models.Meeting.status == "completed")
    ...
    if not meeting_ids:
        return schemas.SearchResponse(
            results=[], total=0, query=query, search_time_ms=(time.time() - start_time) * 1000
        )

    # Search in transcripts
    if "transcripts" in search_query.search_in:
        transcripts = (
            db.query(meeting_models.Transcription)
            .filter(
    ...
    if "action_items" in search_query.search_in:
        ...
        trans_query = (
            db.query(meeting_models.Transcription.id, meeting_models.Transcription.meeting_id)
        ...
        if trans_ids:
            action_items = (
                db.query(meeting_models.ActionItem)
                .filter(
    ...
    if "notes" in search_query.search_in:
        notes_results = (
            ...
            .filter(meeting_models.Meeting.id.in_(meeting_ids), meeting_models.Meeting.notes.ilike(search_pattern))
            .all()
        )
    ...
    title_matches = (
        db.query(meeting_models.Meeting)
        .filter(meeting_models.Meeting.id.in_(meeting_ids), meeting_models.Meeting.filename.ilike(search_pattern))
        .all()
    )
    ...
    return schemas.SearchResponse(
        results=limited_results, total=len(unique_results), query=query, search_time_ms=round(search_time, 2)
    )
...
def quick_search(q: str = Query(..., min_length=1), limit: int = Query(10, le=50), db: Session = Depends(get_db)):
    ...
    meetings = (
        db.query(meeting_models.Meeting)
        .filter(meeting_models.Meeting.status == "completed", meeting_models.Meeting.filename.ilike(search_pattern))
        .limit(limit)
        .all()
    )
    ...
    if len(results) < limit:
        ...
        action_items = (
            db.query(meeting_models.ActionItem)
            .filter(meeting_models.ActionItem.task.ilike(search_pattern))
            .limit(remaining)
            .all()
        )
        ...
        for ai in action_items:
            trans = (
                db.query(meeting_models.Transcription)
                .filter(meeting_models.Transcription.id == ai.transcription_id)
                .first()
            )
            if trans:
                meeting = db.query(meeting_models.Meeting).filter(meeting_models.Meeting.id == trans.meeting_id).first()
                if meeting:
                    results.append(
                        {
                            "id": ai.id,
