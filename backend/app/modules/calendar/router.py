...
def get_scheduled_meetings(
    ...
    scheduled_meetings = crud.get_scheduled_meetings(
        db, skip=skip, limit=limit, status=status, upcoming_only=upcoming_only
    )
...
def get_scheduled_meeting(scheduled_meeting_id: int, db: Session = Depends(get_db)):
...
def create_scheduled_meeting(scheduled_meeting: schemas.ScheduledMeetingCreate, db: Session = Depends(get_db)):
    """
...
def update_scheduled_meeting(
    scheduled_meeting_id: int, scheduled_meeting_update: schemas.ScheduledMeetingUpdate, db: Session = Depends(get_db)
):
    """
...
def delete_scheduled_meeting(scheduled_meeting_id: int, db: Session = Depends(get_db)):
    """
...
def link_meeting_to_scheduled(scheduled_meeting_id: int, meeting_id: int, db: Session = Depends(get_db)):
    """
...
def sync_meetings_from_google_calendar(
    ...
    try:
        ...
        events = calendar_service.fetch_upcoming_events(max_results=100, time_min=time_min, time_max=time_max)
...
def get_meetings_needing_upload(
    days_back: int = Query(7, description="How many days back to check"), db: Session = Depends(get_db)
):
    """
