...
def create_chat_message(db: Session, meeting_id: int, role: str, content: str):
    ...
    db_message = models.ChatMessage(meeting_id=meeting_id, role=role, content=content)
    db.add(db_message)
    db.commit()
...
def clear_chat_history(db: Session, meeting_id: int):
    ...
    db.query(models.ChatMessage).filter(models.ChatMessage.meeting_id == meeting_id).delete()
    db.commit()
...
def list_global_chat_sessions(db: Session, skip: int = 0, limit: int = 100):
    ...
    """
    return (
        db.query(models.GlobalChatSession)
        .order_by(models.GlobalChatSession.updated_at.desc())
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def create_global_chat_session(
    ...
    session = models.GlobalChatSession(
        title=title or "New chat", tags=tags, filter_folder=filter_folder, filter_tags=filter_tags
    )
    db.add(session)
    db.commit()
    db.refresh(session)
...
def update_global_chat_session(
    db: Session,
    session_id: int,
    title: str | None = None,
    tags: str | None = None,
...
def add_global_chat_message(db: Session, session_id: int, role: str, content: str, sources: list | None = None):
    message = models.GlobalChatMessage(session_id=session_id, role=role, content=content, sources=sources or [])
    db.add(message)
    session = get_global_chat_session(db, session_id)
    if session:
        session.updated_at = func.now()
