...
async def list_diary_entries(
    ...
    total = DiaryRepository.count_entries(
        db,
        start_date=start_date,
        end_date=end_date
    )
    
    return DiaryEntriesListResponse(
        entries=entries,
        total=total,
        page=page,
        page_size=page_size
...
async def create_diary_entry(
    ...
    if existing_entry:
        raise HTTPException(
            status_code=400,
            detail=f"Diary entry already exists for {entry_data.date}"
        )
...
async def dismiss_diary_reminder(
    dismiss_request: ReminderDismissRequest,
    db: Session = Depends(get_db)
):
...
async def snapshot_action_items(
    entry_date: date,
    db: Session = Depends(get_db)
):
    ...
    if not entry:
        entry_data = DiaryEntryCreate(
            date=entry_date,
            is_work_day=DiaryService.is_work_day(entry_date)
        )
        entry = DiaryRepository.create_entry(db, entry_data)
...
async def get_statistics_summary(
    ...
    # Total action items completed
    total_completed = sum(
        len(e.action_items_completed or []) for e in entries
    )
    ...
    productive_days = sorted(
        [(e.date, len(e.action_items_completed or [])) for e in entries],
        key=lambda x: x[1],
        reverse=True
    )[:5]
