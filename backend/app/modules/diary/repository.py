...
from .models import DiaryActionItemSnapshot, DiaryEntry
...
class DiaryRepository:
    ...
    @staticmethod
    def get_entry_by_date(db: Session, entry_date: date) -> DiaryEntry | None:
        """Get diary entry for a specific date."""
        return db.query(DiaryEntry).filter(DiaryEntry.date == entry_date).first()

    @staticmethod
    def get_entry_by_id(db: Session, entry_id: int) -> DiaryEntry | None:
        """Get diary entry by ID."""
        return db.query(DiaryEntry).filter(DiaryEntry.id == entry_id).first()

    @staticmethod
    def get_entries(
        db: Session, start_date: date | None = None, end_date: date | None = None, skip: int = 0, limit: int = 50
    ) -> list[DiaryEntry]:
    ...
    @staticmethod
    def count_entries(db: Session, start_date: date | None = None, end_date: date | None = None) -> int:
    ...
    @staticmethod
    def update_entry(db: Session, entry: DiaryEntry, entry_data: DiaryEntryUpdate) -> DiaryEntry:
    ...
    def dismiss_reminder(db: Session, entry_date: date) -> DiaryEntry:
        ...
        if not entry:
            ...
            entry = DiaryEntry(date=entry_date, reminder_dismissed=True, is_work_day=True)
            db.add(entry)
        else:
    ...
    @staticmethod
    def get_missing_work_day_dates(db: Session, start_date: date, end_date: date, work_days: list[int]) -> list[date]:
        """Get work days without diary entries in date range."""
        # Get all existing entries in range
        existing_entries = (
            db.query(DiaryEntry.date).filter(and_(DiaryEntry.date >= start_date, DiaryEntry.date <= end_date)).all()
        )
...
class DiaryActionItemSnapshotRepository:
    ...
    @staticmethod
    def get_snapshots_for_entry(db: Session, diary_entry_id: int) -> list[DiaryActionItemSnapshot]:
        """Get all action item snapshots for a diary entry."""
        return db.query(DiaryActionItemSnapshot).filter(DiaryActionItemSnapshot.diary_entry_id == diary_entry_id).all()

    @staticmethod
    def get_snapshots_for_date(db: Session, entry_date: date) -> list[DiaryActionItemSnapshot]:
