...
def get_graph_data(db: Session = Depends(get_db)):
    ...
    try:
        ...
        for link in meeting_links:
            ...
            if any(node["id"] == source_node_id for node in nodes) and any(
                node["id"] == target_node_id for node in nodes
            ):
                # Check if edge doesn't already exist
                edge_exists = any(
                    edge["source"] == source_node_id
                    and edge["target"] == target_node_id
...
def create_meeting_link(meeting_id: int, target_meeting_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Create new link
        new_link = models.MeetingLink(source_meeting_id=meeting_id, target_meeting_id=target_meeting_id)
        db.add(new_link)
        db.commit()
        db.refresh(new_link)
