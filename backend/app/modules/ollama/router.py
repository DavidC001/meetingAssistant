...
def get_docker_command() -> str | None:
...
async def get_ollama_status():
    ...
    return OllamaStatus(status=status, message=messages.get(status, "Unknown status"))
...
async def start_ollama(config: OllamaConfig):
    ...
    try:
                ...
                if "\t" in line:
                    ...
                    if name == "ollama":
                        if state == "running":
                            ...
                            result = subprocess.run(
                                [docker_cmd, "start", "ollama"], capture_output=True, text=True, check=False
                            )
        ...
        if result.returncode == 0:
            ...
            try:
                subprocess.run(
                    [docker_cmd, "exec", "ollama", "ollama", "pull", config.model], capture_output=True, check=False
                )
            except Exception as e:
                print(f"Warning: Could not pull model {config.model}: {e}")

            return {"message": f"Ollama container started successfully with model {config.model}", "status": "starting"}
        else:
            raise HTTPException(status_code=500, detail=f"Failed to start Ollama container: {result.stderr}")
...
async def stop_ollama():
    ...
    try:
        ...
        result = subprocess.run([docker_cmd, "stop", "ollama"], capture_output=True, text=True, check=False, timeout=30)

        if result.returncode == 0:
            return {"message": "Ollama container stopped successfully", "status": "stopped"}
        else:
            raise HTTPException(status_code=500, detail=f"Failed to stop Ollama container: {result.stderr}")
...
async def remove_ollama():
    ...
    try:
        ...
        result = subprocess.run([docker_cmd, "rm", "ollama"], capture_output=True, text=True, check=False)

        if result.returncode == 0:
            return {"message": "Ollama container removed successfully"}
        else:
            raise HTTPException(status_code=500, detail=f"Failed to remove Ollama container: {result.stderr}")
