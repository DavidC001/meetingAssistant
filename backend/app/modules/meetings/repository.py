...
class MeetingRepository(BaseRepository[models.Meeting, schemas.MeetingCreate, schemas.MeetingUpdate]):
    ...
    def get_by_id(self, meeting_id: int) -> Optional[models.Meeting]:
        """Get meeting by ID."""
    ...
    def get_by_filters(
        ...
        """
        query = self.db.query(models.Meeting.id).filter(
        ...
        if tags:
            ...
            if tag_list:
                tag_conditions = [
                    models.Meeting.tags.like(f'%{tag}%') 
                    for tag in tag_list
                ]
    ...
    def delete_meeting(self, meeting_id: int) -> Optional[models.Meeting]:
        ...
        processing_statuses = [
            ...
            models.MeetingStatus.PENDING.value
        ]
        if meeting.celery_task_id and meeting.status in processing_statuses:
            try:
                ...
                logging.getLogger(__name__).warning(
                    f"Failed to cancel Celery task {meeting.celery_task_id}: {e}"
                )
        
        # Delete related diarization timings
        self.db.query(models.DiarizationTiming).filter(
            models.DiarizationTiming.meeting_id == meeting_id
        ).delete()
...
class ActionItemRepository(BaseRepository[models.ActionItem, schemas.ActionItemCreate, schemas.ActionItemUpdate]):
    ...
    def get_by_transcription(
        self,
        transcription_id: int
    ) -> List[models.ActionItem]:
        """Get all action items for a transcription."""
        return self.db.query(models.ActionItem).filter(
            models.ActionItem.transcription_id == transcription_id
        ).all()
    
    def create_action_item(
        self,
        transcription_id: int,
        item_data: schemas.ActionItemCreate,
        is_manual: bool = True
    ) -> models.ActionItem:
...
class AttachmentRepository(BaseRepository[models.Attachment, Any, Any]):
    ...
    def get_by_meeting(self, meeting_id: int) -> List[models.Attachment]:
        ...
        return self.db.query(models.Attachment).filter(
            models.Attachment.meeting_id == meeting_id
        ).all()
...
class TranscriptionRepository(BaseRepository[models.Transcription, schemas.TranscriptionCreate, Any]):
    ...
    def get_by_meeting(self, meeting_id: int) -> Optional[models.Transcription]:
        """Get transcription for a meeting."""
        return self.db.query(models.Transcription).filter(
...
class DocumentChunkRepository(BaseRepository[models.DocumentChunk, Any, Any]):
    ...
    def get_by_meeting(
        ...
        return self.db.query(models.DocumentChunk).filter(
            models.DocumentChunk.meeting_id == meeting_id
        ).order_by(asc(models.DocumentChunk.chunk_index)).all()
    
    def clear_meeting_chunks(self, meeting_id: int) -> int:
        """Delete all chunks for a meeting. Returns count deleted."""
        count = self.db.query(models.DocumentChunk).filter(
            models.DocumentChunk.meeting_id == meeting_id
        ).delete()
        self.db.commit()
        return count
...
class DiarizationTimingRepository(BaseRepository[models.DiarizationTiming, Any, Any]):
    ...
    def get_recent(self, limit: int = 20) -> List[models.DiarizationTiming]:
        ...
        return self.db.query(models.DiarizationTiming).order_by(
            models.DiarizationTiming.created_at.desc()
        ).limit(limit).all()
    
    def get_average_rate(self, limit: int = 10) -> Optional[float]:
        """
        ...
        rates = [
            t.processing_time_seconds / t.audio_duration_seconds
            for t in timings
            if t.audio_duration_seconds > 0
        ]
...
class SpeakerRepository(BaseRepository[models.Speaker, Any, Any]):
    ...
    def get_unique_names(self) -> List[str]:
        ...
        results = self.db.query(models.Speaker.name).distinct().order_by(
            models.Speaker.name
        ).all()
