...
def get_app_settings():
    ...
    return {"maxFileSize": max_file_size, "defaultMaxFileSize": default_max_file_size}
...
def update_api_key(key_id: int, api_key_update: schemas.APIKeyUpdate, db: Session = Depends(get_db)):
    ...
    if api_key_update.key_value:
        ...
        env_var_name = (
            api_key_update.environment_variable
            if api_key_update.environment_variable
            else current_api_key.environment_variable
        )
...
def create_embedding_configuration(config: schemas.EmbeddingConfigurationCreate, db: Session = Depends(get_db)):
    ...
    if dimension is not None and (
        not hasattr(config, "dimension") or config.dimension is None or config.dimension == 0
    ):
...
def update_embedding_configuration(
    ...
    # Auto-populate dimension if not provided and detected
    if dimension is not None and (
        not hasattr(payload, "dimension") or payload.dimension is None or payload.dimension == 0
    ):
