...
try:
    ...
    from app.core.processing.transcript_formatter import convert_old_transcript_format, format_transcript_grouped
except ImportError:
...
def print_comparison():
    ...
    results = [
        ...
        {
            "speaker": "Fausto Giunchiglia",
            "start": 6.41,
            "end": 9.23,
            "text": "Lo decomponiamo in questa maniera, vai su.",
        },
    ...
    old_format = "\n".join([f"{r['speaker']} ({r['start']:.2f}s - {r['end']:.2f}s): {r['text']}" for r in results])
